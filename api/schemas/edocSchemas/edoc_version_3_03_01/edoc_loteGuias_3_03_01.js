...
const identEquipeSchema = joi.object().keys({
	...
	nomeProf: joi.string().min(1).max(70).required(),
	conselho: joi.string().valid([lists.conselhoProfissional]),
	numeroConselhoProfissional: joi.string().min(1).max(15),
	UF: joi.string().valid(lists.UF),
	CBOS: joi.string().valid(lists.CBOS)
})

const identEquipeSadtSchema = identEquipeSchema.keys({
...
const procedimentosExecutadosSchema = joi.object().keys({
	procedimentoExecutado: joi.object().keys({
		...
		quantidadeExecutada: joi.string().regex(/^\d{1,3}?$/),
		...
		equipeSadt: joi.array().items(identEquipeSadtSchema)
	}).required()
})

const outrasDespesasSchemas = joi.object().keys({
	despesa: joi.object().keys({
		codigoDespesa:joi.string().valid(["01","02","03","05","07","08"]).required(),
		servicosExecutados: joi.object().keys({
			...
			unidadeMedida: joi.string().valid(lists.unidadeMedida).required(),
			reducaoAcrescimo: joi.string().regex(/^\d{1,3}(\.\d{0,2})?$/).required(),
			...
			registroANVISA: joi.string().min(1).max(15),
			codigoRefFabricante: joi.string().min(1).max(60),
			autorizacaoFuncionamento: joi.string().min(1).max(30)
		}).required()
	})
})
...
const guiaSPSADTSchema = joi.object().keys({
	...
	procedimentosExecutados: joi.array().items(procedimentosExecutadosSchema),
	outrasDespesas: joi.array().items(outrasDespesasSchemas),
...
const identEquipeIntSchema = identEquipeSchema.keys({
	grauPart: joi.string().valid(["01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12", "13"]).required(),
})
...
const procedimentosExecutadosIntSchema = joi.object().keys({
	procedimentoExecutado: joi.object().keys({
		...
		valorUnitario: joi.string().regex(/^\d{1,8}(\.\d{0,2})?$/).required(),
		valorTotal: joi.string().regex(/^\d{1,8}(\.\d{0,2})?$/).required(),
		identEquipe: joi.array().items(identEquipeIntSchema)
	}).required()
})
